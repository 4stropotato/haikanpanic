...
const IsoGrid = ({ show }) => {
  ...
  useEffect(() => {
    ...
    const drawLine = (x1, y1, x2, y2, bold = false) => {
      ...
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.strokeStyle = bold ? "#4cc1f7" : "#3aa0d8"; // v1.0 use bold color for main lines
      ctx.lineWidth = bold ? 1.2 : 0.4;              // v1.0 thicker line for major grid
      ctx.stroke();
    };
    ...
    const drawIsoGrid = () => {
      ...
      if (!show) return;                     // v1.01 respect `show` prop
      ...
      // Slanted grid lines (right-leaning)
      for (let i = -cols; i < cols * 2; i++) {
        const x = i * dx;
        const bold = i % 10 === 0; // v1.0 every 10th line bold
        drawLine(x, 0, x + height / Math.tan(Math.PI / 6), height, bold);
        drawLine(x, 0, x + height / Math.tan(Math.PI / 6), -height, bold);
      }

      // Slanted grid lines (left-leaning)
      for (let i = -cols; i < cols * 2; i++) {
        const x = i * dx;
        const bold = i % 10 === 0; // v1.0 every 10th line bold
        drawLine(x, 0, x - height / Math.tan(Math.PI / 6), height, bold);
      ...
      drawCenterDot();            // v1.0 add center dot after grid
      ctx.restore();
    };
  ...
  }, [show]); // v1.01 added `show` as dependency to redraw when toggled
