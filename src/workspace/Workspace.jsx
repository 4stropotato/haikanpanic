...
import {
  ...
  groundCoordsOf, upPerMmOf, datumHeightMm,                               // v4.00 height in, height out
    ...
    setDatums((list) => list.map((d, i) => (i === pending
      ? {
        ...
        centerAB: groundCoordsOf({ x: plane.cx, y: plane.cy }, planeAxes(view),
          datumHeightMm(d), upPerMmOf(view, plane.pxPerMm)),
        fitted: true,
      }
      : d)));
...
const ZOOM_MAX = 6;
  ...
  const ZOOM_CEIL_PCT = 10000;     // 100m across the screen
  const spanPerZoom = () => {
  ...
  // as far out as it goes: 10000%, one hundred metres across
...
const nodeKey = (p) => `${p.x.toFixed(3)},${p.y.toFixed(3)}`;
  ...
  // v4.25 It holds, then fades out rather than blinking away — a value that
  // vanishes between one glance and the next is worse than none.
  const showZoom = (z) => {
    setZoomReadout({ zoom: z, going: false });
    if (zoomFade.current) clearTimeout(zoomFade.current);
    zoomFade.current = setTimeout(() => {
      setZoomReadout((r) => (r ? { ...r, going: true } : null));
      zoomFade.current = setTimeout(() => setZoomReadout(null), 500);
    }, 1000);
  };
    ...
    for (const i of drag.datums ?? []) {
      ...
      patchDatum(i, {
        centerAB: groundCoordsOf({ x: base.x + dx, y: base.y + dy }, planeAxes(view),
          datumHeightMm(datums[i] ?? {}), upPerMmOf(view, movedPlane?.pxPerMm ?? 0)),
      });
    }
    setLines(lines.map((line, i) => {
      if (!drag.members.has(i)) return line;
  ...
  const planeMove = (clientX, clientY) => {
    ...
    if (drag.mode === "resize") {
      ...
      const ab = groundCoordsOf(moved, planeAxes(view), datumHeightMm(datums[movedIdx] ?? {}),
        upPerMmOf(view, movedPlane?.pxPerMm ?? 0));
