## [v1.15] 2026-01-16
- **Endpoint Snapping**: Cursor snaps to existing line endpoints with priority over grid
- **Elbow Indicator**: Green piping elbow symbol appears when snapped to an endpoint
- Enables creating connected pipe segments and elbows naturally
- Added `findNearestEndpoint()` utility function in geometry.js
- Added `endpointSnapThreshold` constant (15px)
...
- **No Emojis**: Replaced all emoji icons with clean SVG icons for professional look
- Removed emojis from README and all UI components
